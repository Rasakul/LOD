...
Publishing a LOD system there must be a compromise found between actuality of the data and a proper system load of the interface to the data source (e.g. a active used database), so existing system may not be (to heavily) effected by the LOD system. According to the data the actuality may range between weeks and days (e.g. for location data) or minutes and seconds (e.g. public transport data).

Further it must be ensured that the linking of the data are valid at every time, linking to not longer existing sources or sources with sinking data quality will lower the quality of data itself.

\subsubsection{Response time of the LOD interface}
To achieve not only a providing of the data but also an use of the data in applications, the interface (e.g.